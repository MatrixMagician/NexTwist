...
pub async fn add_game(state: State<'_, Mutex<AppState>>, appid: u32) -> Result<Game, String> {
    let game = steam::resolve_game(appid)
...
pub async fn add_game_by_folder(
    ...
    let game = steam::add_game_by_folder(&path, appid)
        .map_err(boundary_err)?
        .into_game();
    state
        .lock()
...
///
/// Thin forwarder: the engine re-resolves the Steam library + Proton prefix
