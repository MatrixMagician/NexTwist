...
--   * managed_mod  — multi-mod registry per game, with a rank that orders
--                    conflict winners (LOWER rank = HIGHER priority = wins).
--   * profile      — a lightweight reference set over the shared staging
--                    store; exactly one active profile per game.
--   * profile_mod  — per-profile membership: which mods are enabled and at
--                    what rank, independently per profile.
--   * plugin_state — per-profile plugin enable + load-order state.
--
-- Data migration: every existing managed_game gets one active 'Default'
...
-- The mods NexTwist manages for a game. `rank` is 1-based; lower wins.
...
-- Profiles. UNIQUE(appid, name) rejects duplicate names per game.
...
-- Per-profile membership + per-profile rank.
...
-- Data migration: one active 'Default' profile per existing managed game.
