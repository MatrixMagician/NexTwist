...
(`ROADMAP.md`, `STATE.md`, phase dirs). Per the project's GSD enforcement, route
file-changing work through a GSD entry point (`/gsd-quick`, `/gsd-debug`,
`/gsd-execute-phase`) rather than editing directly, unless explicitly told to bypass.
The codebase is graphmind-indexed — use `/gm` for code exploration before grep/Explore.
