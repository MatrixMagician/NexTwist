...
- **The engine is now validated against a real game install, not just fixtures.** Every
  ...
  `plugins.txt` round-trip, and the user-order splice), and the full archive → extract →
  stage → deploy → purge user workflow. They skip cleanly without the sandbox, so CI is unaffected; populate it
