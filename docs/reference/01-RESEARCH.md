...
**Version verification:** All versions above confirmed against `crates.io/api/v1/crates/<name>` max_stable_version on 2026-06-20 and match the pinned values in `docs/reference/STACK.md` and `.claude/CLAUDE.md`. No drift detected.

## Package Legitimacy Audit

Ran a package-legitimacy check on every new crate.
...
> This is a greenfield phase (repo contains only `.claude/`, `LICENSE`, `README.md`). No rename/refactor/migration. The categories below are documented for completeness because the *engine being built* creates persistent runtime state Phase 2+ must respect.
...
(These are the Phase-1-relevant subset of `docs/reference/PITFALLS.md`, deepened with verified detail.)
...
- **Phase gate:** Full suite green + the crash-recovery and round-trip-pristine integration tests passing on **at least tmpfs + the dev btrfs**.
...
- package-legitimacy check — all 12 audited crates verdict OK
