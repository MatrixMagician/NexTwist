...
**Domain:** Linux desktop NexusMods mod manager (Rust + Tauri) for Proton/Wine-run Windows games
**Researched:** 2026-06-20
**Confidence:** HIGH on crate versions (verified against crates.io), MEDIUM on architecture/API recommendations (cross-checked web + official NexusMods.App docs)
...
cargo add tauri --features "protocol-asset"
cargo add tauri-plugin-deep-link tauri-plugin-single-instance tauri-plugin-store
cargo add tokio --features "rt-multi-thread,fs,macros,process"
cargo add reqwest --no-default-features --features "rustls-tls,json,stream,gzip,brotli"
cargo add oauth2 keyring governor
cargo add rusqlite --features "bundled" && cargo add refinery --features "rusqlite"
cargo add serde --features derive && cargo add serde_json
cargo add zip sevenz-rust2 tar xz2 flate2
cargo add reflink-copy walkdir steamlocate keyvalues-serde keyvalues-parser
cargo add tracing tracing-subscriber thiserror anyhow

# Dev / CI
cargo install tauri-cli --version "^2" cargo-deny
```
...
**If the user's mods drive is a separate filesystem from the game (common):**
...
**If the target FS is Btrfs/XFS/bcachefs (CoW-capable):**
...
**If the FS is ext4/other (no reflink):**
...
**If authenticating a brand-new app install:**
...
---
*Stack research for: Linux desktop NexusMods mod manager (Rust + Tauri)*
*Researched: 2026-06-20*
