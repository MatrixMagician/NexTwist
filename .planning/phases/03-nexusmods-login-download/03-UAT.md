...
number: 1
name: Live OAuth2 round-trip login (NEXUS-01)
expected: |
  After a public OAuth client_id + nxm://oauth/callback redirect is registered under the
  Nexus Acceptable Use Policy, running `cargo tauri dev` and completing an OAuth login
  populates the account panel with the real username + tier, and the refresh token lands
  in the OS keyring (never a plaintext file).
