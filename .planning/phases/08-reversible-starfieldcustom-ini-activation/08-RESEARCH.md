...
.planning/phases/08-.../08-SECURITY.md   # NEW (first write outside Data/ — path-containment + INI-injection surface)
...
- **Phase gate:** full workspace green + `cargo clippy --workspace --all-targets -- -D warnings` + `cargo deny check` before `/gsd-verify-work`
